...
public class SimpleTelegramBot extends TelegramLongPollingBot {
    ...
    public void addCommandHandler(String command, InterfaceBotMode method,DialogMode mode) {
        handlerList.add(() -> {
            String messageText = getMessageText();
            if (("/" + command).equals(messageText))
                method.execute(mode);
    ...
    public CallbackQuery getButtonCallback() {
        return updateEvent.get().hasCallbackQuery() ? updateEvent.get().getCallbackQuery() : null;
    }
    /**
     El método envía TEXTO (mensaje de texto) al chat.
     Se admite la sintaxis de markdown.
     si plain true, markdown desactivado
     */
    public Message sendTextMessage(String text,boolean plain) {
        SendMessage command = createApiSendMessageCommand(text);
        if(plain){
            command.enableMarkdown(false);
            command.enableMarkdownV2(false);
        }

        return executeTelegramApiMethod(command);
    }
    public Message sendTextMessage(String text){
        return sendTextMessage(text,false);
    }
    /**
    ...
    public void updateTextMessage(MaybeInaccessibleMessage message, String text) {
        EditMessageText command = new EditMessageText();
        command.setChatId(message.getChatId());
        command.setMessageId(message.getMessageId());
        command.setText(text);
    ...
    private SendMessage createApiSendMessageCommand(String text) {
        SendMessage message = new SendMessage();
        /*String newText = String.format("current Mode: %s\n-----------------\n",currentMode);
        text = newText+ text;*/
        message.setText(new String(text.getBytes(), StandardCharsets.UTF_8));
    ...
    public void attachButtons(SendMessage message, List<String> buttons) {
    ...
    private SendPhoto createApiPhotoMessageCommand(String photoKey, String text) {
        try {
            ...
            logger.warning("photoKey: "+photoKey);
            throw new RuntimeException("Can't create photo message!, "+e.getMessage());
        }
    }
    ...
    public String loadPrompt(String name) {
        try {
            ...
            logger.severe("loadPrompt: "+e.getMessage());
            return "";
    ...
    public static String loadMessage(String name) {
        try {
            ...
            return "No se pudo cargar el mensaje";
    ...
    @FunctionalInterface
    public interface InterfaceBotMode {
        void execute(DialogMode mode);
    }
    public DialogMode getCurrentMode() {
        return currentMode;
    }

    public void setCurrentMode(DialogMode currentMode) {
